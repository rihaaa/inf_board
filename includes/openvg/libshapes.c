...
#include "fonts/TimesNewRoman.inc"
#include "fonts/ComicSans.inc"
...
static const int MAXFONTPATH = 1150;
//
// Terminal settings
//
...
void init(int *w, int *h) {
	...
	TimesTypeface = loadfont(TimesNewRoman_glyphPoints,
				TimesNewRoman_glyphPointIndices,
				TimesNewRoman_glyphInstructions,
				TimesNewRoman_glyphInstructionIndices,
				TimesNewRoman_glyphInstructionCounts,
				TimesNewRoman_glyphAdvances, TimesNewRoman_characterMap, TimesNewRoman_glyphCount);

	ComicTypeface = loadfont(ComicSans_glyphPoints,
				ComicSans_glyphPointIndices,
				ComicSans_glyphInstructions,
				ComicSans_glyphInstructionIndices,
				ComicSans_glyphInstructionCounts,
				ComicSans_glyphAdvances, ComicSans_characterMap, ComicSans_glyphCount);
...
void TextW(VGfloat x, VGfloat y, wchar_t *s, Fontinfo f, int pointsize) {
	VGfloat size = (VGfloat) pointsize, xx = x, mm[9];
	int i;
...
void TextNew(VGfloat x, VGfloat y, int s[], Fontinfo f, int pointsize) {
	VGfloat size = (VGfloat) pointsize, xx = x, mm[9];
	int i;
	...
	for (i = 0; i < 100; i++) {
		...
		int glyph = f.CharacterMap[character];// tested 400 - 401 - ok, 256 = A
		if (glyph == -1) {
			continue;	//glyph is undefined
		}
		VGfloat mat[9] = {
...
VGfloat TextWidthW(wchar_t *s, Fontinfo f, int pointsize) {
	int i;
	VGfloat tw = 0.0;
	VGfloat size = (VGfloat) pointsize;
	for (i = 0; i < (int)wcslen(s); i++) {
...
// TextMidW draws text, centered on (x,y)
void TextMidW(VGfloat x, VGfloat y, wchar_t *s, Fontinfo f, int pointsize) {
	VGfloat tw = TextWidthW(s, f, pointsize);
	TextW(x - (tw / 2.0), y, s, f, pointsize);
}

// TextEndW draws text, with its end aligned to (x,y)
void TextEndW(VGfloat x, VGfloat y, wchar_t *s, Fontinfo f, int pointsize) {
	VGfloat tw = TextWidthW(s, f, pointsize);
	TextW(x - tw, y, s, f, pointsize);
}
...
void DoSnapshot(int w, int h, int s, int e, char *filename)
{
